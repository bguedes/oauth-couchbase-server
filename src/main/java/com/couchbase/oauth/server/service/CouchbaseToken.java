...
import com.couchbase.oauth.server.repository.CouchbaseAccessTokenRepository;
import com.couchbase.oauth.server.repository.CouchbaseRefreshTokenRepository;

import lombok.extern.slf4j.Slf4j;

@Service
@Slf4j
public class CouchbaseToken implements TokenStore {
    ...
    public CouchbaseToken(CouchbaseAccessTokenRepository cbAccessTokenRepository, 
    		CouchbaseRefreshTokenRepository cbRefreshTokenRepository){
	...
	public OAuth2Authentication readAuthentication(String tokenValues) {
        Optional<CouchbaseAccessToken> accessToken = cbAccessTokenRepository.findByTokenId(extractTokenKey(tokenValues));
        if (accessToken.isPresent()) {
	...
	public void removeAccessToken(OAuth2AccessToken accessToken) {
        Optional<CouchbaseAccessToken> accessTokenToRemove = cbAccessTokenRepository.findByTokenId(extractTokenKey(accessToken.getValue()));
